...
export default function OTP() {
  ...
  const [email, setEmail] = useState(
    location.state?.email || localStorage.getItem('userEmailForOtp')
  );
        ...
        >
          ...
          <div className="backdrop-blur-3xl bg-white/40 rounded-3xl shadow-2xl border border-white/50 p-10">
            ...
            <div className="text-center mb-10">
              <h1 className="text-3xl font-bold tracking-wider mb-2 text-gray-800">
                VERIFICATION
              </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex justify-center gap-2 sm:gap-4 py-4">
                {otp.map((digit, index) => (
                  <input
                    ...
                    className="w-12 h-14 sm:w-14 sm:h-16 text-center text-2xl font-extrabold rounded-xl bg-white/70 border text-gray-800 focus:outline-none focus:ring-4 focus:ring-cyan-300/40 shadow-md"
                    required
                  />
                ))}
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-4 rounded-full bg-gradient-to-r from-[#1a9bbf] to-cyan-500 text-white font-bold text-xl flex items-center justify-center gap-2 transition-all duration-300 hover:scale-105 disabled:grayscale"
              >
                {isLoading ? (
                  <>
